...
class RasterIOfull:
    ...
    def getValue(self, row, col):
        ...
        if self.block is None or col < self.xblock or col >= self.xblock + self.blocksize or row < self.yblock or row >= self.yblock + self.blocksize:
            ...
            mx = self.raster.extent.XMin + (self.xblock+0.5)*self.raster.meanCellWidth
            my = max(self.raster.extent.YMin, self.raster.extent.YMax - (self.blocksize + self.yblock)*self.raster.meanCellHeight) + 0.5*self.raster.meanCellHeight
...
if __name__ == "__main__":
    ...
    if raster is not None:
        myData = arcpy.RasterToNumPyArray(raster, arcpy.Point(mx+0.5*rasterlike.meanCellWidth, my+0.5*rasterlike.meanCellHeight),
